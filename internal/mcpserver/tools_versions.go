...
func (s *Server) versions(ctx context.Context, _ *mcp.CallToolRequest, input pkgsite.VersionsInput) (*mcp.CallToolResult, any, error) {
	...
	return s.result(result, input.PageInput, toolNameVersions, map[string]any{"module_path": input.ModulePath, "limit": input.Limit, "token": input.Token, "filter": input.Filter}, err)
}
